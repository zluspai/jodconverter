...
public class DefaultOfficeManagerConfiguration {
	 ...
	 * <p>
	 * The default is to use {@link SigarProcessManager} if sigar.jar is available in the classpath, otherwise {@link LinuxProcessManager} on Linux
	...
	public OfficeManager buildOfficeManager() throws IllegalStateException {
		...
		final int numInstances = connectionProtocol == OfficeConnectionProtocol.PIPE ? pipeNames.length : portNumbers.length;
		...
		return new ProcessPoolOfficeManager( officeHome, unoUrls, runAsArgs, templateProfileDir, workDir, retryTimeout, taskQueueTimeout,
				taskExecutionTimeout, maxTasksPerProcess, processManager, manageExternalInstances );
	}
