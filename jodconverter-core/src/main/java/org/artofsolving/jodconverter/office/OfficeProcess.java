...
class OfficeProcess {
	...
	private final boolean manageExternalInstances;

	public OfficeProcess( final File officeHome, final UnoUrl unoUrl, final String[] runAsArgs, final File templateProfileDir, final File workDir,
			final ProcessManager processManager, final boolean manageExternalInstances ) {
		this.officeHome = officeHome;
		this.unoUrl = unoUrl;
		this.runAsArgs = runAsArgs;
		this.templateProfileDir = templateProfileDir;
		instanceProfileDir = getInstanceProfileDir( workDir, unoUrl );
		this.processManager = processManager;
		this.manageExternalInstances = manageExternalInstances;
	}
	...
	public void start( final boolean restart ) throws IOException {
		final ProcessQuery processQuery = new ProcessQuery( "soffice.bin", unoUrl.getAcceptString() );
		final long existingPid = processManager.findPid( processQuery );
		if ( !manageExternalInstances && !( existingPid == PID_NOT_FOUND || existingPid == PID_UNKNOWN ) )
			throw new IllegalStateException( String.format( "a process with acceptString '%s' is already running; pid %d", unoUrl.getAcceptString(),
					existingPid ) );
		if ( !restart )
	...
	public void deleteProfileDir() {
		if ( instanceProfileDir != null )
			try {
				...
				final File oldProfileDir = new File( instanceProfileDir.getParentFile(), instanceProfileDir.getName() + ".old."
	...
	private void addBasisAndUrePaths( final ProcessBuilder processBuilder ) throws IOException {
		...
		final File basisLink = new File( officeHome, "basis-link" );
		if ( !basisLink.isFile() ) {
		...
		final File basisHome = new File( officeHome, basisLinkText );
