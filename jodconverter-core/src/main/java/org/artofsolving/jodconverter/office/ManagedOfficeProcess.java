...
class ManagedOfficeProcess {
	...
	public ManagedOfficeProcess( final ManagedOfficeProcessSettings settings ) throws OfficeException {
		...
		process = new OfficeProcess( settings.getOfficeHome(), settings.getUnoUrl(), settings.getRunAsArgs(), settings.getTemplateProfileDir(),
				settings.getWorkDir(), settings.getProcessManager(), settings.isManageExternalInstances() );
	...
	private void doStopProcess() {
		try {
			final XDesktop desktop = OfficeUtils.cast( XDesktop.class, connection.getService( OfficeUtils.SERVICE_DESKTOP ) );
			desktop.terminate();
	...
	private void doEnsureProcessExited() throws OfficeException {
		try {
			final int exitCode = process.getExitCode( settings.getRetryInterval(), settings.getRetryTimeout() );
	...
	private void doTerminateProcess() throws OfficeException {
		try {
			final int exitCode = process.forciblyTerminate( settings.getRetryInterval(), settings.getRetryTimeout() );
